...
for (i in 18:70){
     Feminino[(i-17)]=sum(DadosServidores$x[DadosServidores$x==i & DadosServidores$Sexo==1])/i
     Masculino[(i-17)]=sum(DadosServidores$x[DadosServidores$x==i & DadosServidores$Sexo==2])/i
}

amplitude=1
edadmax=70

escalax<-length(DadosServidores$x)/500
max1<-max(c(Feminino,Masculino));n<-length(Masculino)
min.x<--(max1%/%escalax+1)*escalax;max.x<-(max1%/%escalax+1)*escalax

pir<-plot(0,0,type="n",xaxt='n',yaxt='n',ylim=c(18,edadmax+5),xlim=c(min.x,max.x),xlab="",ylab="")
...
title(main=paste("\n",titulo),ylab="Idade", xlab="Frequência")
legend(x.l1,edadmax+7.5,"Feminino",bty="n",xjust=1)
legend(x.l2,edadmax+7.5,"Masculino",bty="n")
}
